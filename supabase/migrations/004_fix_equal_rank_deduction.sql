-- ============================================================
-- 004_fix_equal_rank_deduction.sql
-- 修复：档次相等时次数扣除被回滚的问题
-- ============================================================

-- 新增：处理档次不高的情况（仅扣次数）
create or replace function execute_draw_rejected(
  p_participant_id uuid,
  p_old_prize_level prize_level
) returns void as $$
begin
  -- 扣除抽奖次数
  update participants
...
begin
  ...
  -- 5. 比较档次（不再扣除次数，让前端调用 execute_draw_rejected）
  if found and v_new_rank <= v_old_rank then
    -- 抛出异常，让前端调用 execute_draw_rejected
